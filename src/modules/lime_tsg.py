...
directly into the transceiver's digital signal chain, via the "TSG_NCO" and "TSP_CONST" writeSettings. Used
together with lime_loopback.py to generate a known reference tone for spectrum-analyzer amplitude calibration.
...
both listing these as per-channel "Other Settings"):
    TSG_NCO   - selects/enables an NCO-generated tone: -1 disables it; 4 places it at approximately
                ...
                be chip/register-revision dependent -- see the note in enable() and lime_loopback.py, which
...
Requires an already-open SoapySDR device handle (e.g. SoapySDRReceiver.device, once the receiver has been
started) -- this module does not open or manage its own device connection.
"""
...
class LimeTestSignalGenerator:
    ...
    Typical usage (TX-side tone, looped back into RX for calibration -- see lime_loopback.py):

        from SoapySDR import SOAPY_SDR_TX
        tsg = LimeTestSignalGenerator(receiver.device, direction=SOAPY_SDR_TX, channel=0)
    ...
    def expected_tone_offset_hz(sample_rate_hz: float, divisor: int) -> float:
        ...
        assumed here -- callers (see lime_loopback.py) should search both +/- this offset in a captured spectrum
