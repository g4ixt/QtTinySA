...
class analyser:
        ...
        if self.usb is not None:
            if self.sweeping:  # if it's running, stop it
                ...
                try:  # start measurements
                    ...
                    # activeButtons(False)
                    self.runButton('Stop')
                    # threadpool.start(self.usbThread)
                    self.usbSend()
    ...
    def setPoints(self):  # may be called by thread
        if ui.points_auto.isChecked():
            ...
            if preferences.bestPoints.isChecked():
                self.points = int((ui.span_freq.value()*1000)/(float(self.rbw)/3))  # best power accuracy; freq in kHz
            else:
                self.points = int((ui.span_freq.value()*1000)/(float(self.rbw)/2))  # normal power accuracy; freq in kHz
            logging.debug(f'before clip points = {self.points}')
            self.points = np.clip(self.points, preferences.minPoints.value(), preferences.maxPoints.value())  # limits
        # else:
        #     self.points = ui.points_box.value()
    ...
    def measurement(self, f_low, f_high):  # runs in a separate thread
        ...
        while self.sweeping:
            try:
                ...
                command = f'scanraw {int(f_low)} {int(f_high)} {int(self.points)}\r'.encode()
                self.usb.write(command)
                index = 0
                self.runTimer.start()  # block test
                self.usb.read_until(command + b'\n{')  # skip command echo
                dataBlock = ''
                while dataBlock != b'}ch' and index < self.points:  # if '}ch' it's reached the end of the scan points
                    ...
                    if dataBlock != b'}ch':
                        ...
                        if index // 20 == index / 20 or index == (self.points - 1):  # update traces every 20 readings
                ...
                self.scanCount += 1
                self.signals.result3D.emit(self.sweepresults)  # update 3D only once per sweep, for gui performance
                logging.debug(f'measurement: sweepresults = {self.sweepresults}')
    ...
    def sigProcess(self, signal):  # signal is emitted from the worker thread
        ...
        S1.updateTrace(options.get(S1.traceType))
        S2.updateTrace(options.get(S2.traceType))
        S3.updateTrace(options.get(S3.traceType))
        S4.updateTrace(options.get(S4.traceType))
    ...
    def updateGUI(self, results):
        ...
        # update signal peak values once per scan
        ...
        logging.debug(f'updategui: peaksort = {peaksort[:20]}')
        ...
        S1.updateMarker(signalAvg)
        S2.updateMarker(signalAvg)
        S3.updateMarker(signalAvg)
        S4.updateMarker(signalAvg)
...
def rbwChanged():
    if ui.rbw_box.currentIndex() == 0:  # can't calculate Points because we don't know what the RBW will be
        ...
        ui.points_auto.setEnabled(True)  # thread
    tinySA.setRBW()


def pointsChanged():
    tinySA.points = ui.points_box.value()
    tinySA.setPoints()
